...
import { Box, Card, Link, Stack, Typography, Button, Grid, useTheme, Skeleton } from '@mui/material';
...
const ExploreRooms = ({ filterName = '' }) => {
    ...
    const isParticipant = useCallback((room) => {
    ...
    return (
        <ThemeProvider theme={exploreRoomsTheme}>
                        ...
                        <Grid item key={room.slug} xs={12} sm={6} md={4} lg={3}>
                            <Card>
                                ...
                                <Stack spacing={2} sx={{ padding: theme.spacing(2) }}>
                                    ...
                                    >
                                        {truncateText(room.description, 30)} {/* Adjust 100 to the desired character limit */}
                                    </Typography>
                                    <Stack direction="row" alignItems="center" justifyContent="space-between">
                                        <div className='cursor-pointer text-sm text-[#1877F2] font-medium' onClick={() => handleJoinRoom(room)}>
                                            {actions[room.slug]}
                                        </div>
